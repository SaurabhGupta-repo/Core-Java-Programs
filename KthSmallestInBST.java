...
public classs KthSmallestInBST{
...
inOrderTraversal(t, nums, k);

return nums[1];
}

void inOrderTraversal( Tree<Integer>t, int [] nums, int k)
{
    
    if(t==null)
        return;
    
    System.out.println("inOrderTraversal left "+ nums[0]);
    inOrderTraversal(t.left, nums, k);
    
    if(++nums[0] == k)
    {
     System.out.println("SUccess: "+ nums[0]);
        nums[1] = t.value;
        return;
    }
    
        System.out.println("inOrderTraversal right  "+ nums[0]);
    inOrderTraversal(t.right, nums, k);
